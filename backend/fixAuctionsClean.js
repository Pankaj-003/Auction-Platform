...
async function fixAuctions() {
  try {
    ...
    // Define models directly without imports to avoid circular dependencies
    const User = mongoose.model('User', new mongoose.Schema({}), 'users');
    const Auction = mongoose.model('Auction', new mongoose.Schema({}), 'auctions');
    ...
    console.log(`Using user "${defaultSeller.name || defaultSeller.email}" (${defaultSeller._id}) as default seller`);
